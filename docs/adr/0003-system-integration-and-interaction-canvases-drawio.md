...
- **Format**: `.drawio` (mxGraph XML) — widely supported (desktop app, VS Code extension, Confluence/Jira plugins, GitHub/GitLab preview), so exported diagrams stay useful outside ParallAlign.
...
- **Negative**: draw.io is a general-purpose diagram editor, not purpose-built for either "integration architecture" or "screen storyboard" — ParallAlign will likely need to curate/restrict the shape palette per canvas to keep each "deliberately incomplete" per the README's intent, rather than exposing draw.io's full generic toolset.
