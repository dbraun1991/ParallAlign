...
**Current status: full ADR-0018/0019 data model + copy, real persistence, dark-mode canvases, working All view.** Product framing (`README.md`) and architecture decisions (`docs/adr/`) are in place; the Vite pipeline (ADR-0012), the Issue-shell (`src/shell/`, ADR-0007/0008/0013), the light/dark toggle (ADR-0014) — including System/Interaction (draw.io) and Object (Mermaid) per ADR-0015, but Process (bpmn-js) deliberately stays light-only per ADR-0016 — real git-backed persistence (`src/persistence/`, ADR-0009 — `isomorphic-git`/`lightning-fs` over IndexedDB, `schemaVersion: 2`, one JSON file per Issue, every save a commit), the All view (`src/canvases/thumbnails.js`, ADR-0017 — real per-engine SVG thumbnails, click-through, content-hash cached, not yet live-updating), New-Issue creation, per-view history browsing/restore (ADR-0009's other half), and ADR-0018/0019's revised model — Issue is just `{id, name, status}` (shown in the view-switcher tab bar, not the Backlog panel), Backlog is a `backlogEntries` list (add/edit/delete, no per-entry status), cross-issue copy covers views (overwrite) and Backlog entries (list-append), never a whole Issue — are all working end to end. Three example Issues still seed automatically on a true first run (`src/persistence/seed-issues.js`, ADR-0018 shape), now alongside any Issues created since.
...
Persistence — one JSON document per Issue (ADR-0009, schema per ADR-0018):
  ...
  The Issue itself is just name+status now (ADR-0018) — theme/notes, which
  used to live on the Issue, are gone; Backlog is a list of small entries
  instead of one field-set, each with its own id, no status field (an entry
  either exists or is deleted, no in-between). Every view still carries its
...
Cross-issue copy (ADR-0019, supersedes ADR-0010): a whole Issue is never
  copyable. Views copy by overwrite (unchanged from ADR-0010: source's
  current HEAD, destination keeps its own view id, copiedFrom provenance).
  Backlog entries copy by list-append instead (a new entry, new id, cloned
  name/description, copiedFrom anchored on the source entry's own id) —
  never overwrite, since entries are now a list, not a singleton field.
```
...
| `docs/adr/0005-*.md` | Backlog Canvas: custom list component (item/state model superseded by 0018) |
...
| `docs/adr/0009-*.md` | Issue persistence: one JSON document per Issue, git-backed per-view history (Issue-schema portion superseded by 0018) |
| `docs/adr/0010-*.md` | Cross-issue copy: always latest version, overwrite, git-recorded provenance (superseded by 0019) |
...
| [0005](docs/adr/0005-backlog-canvas-custom-list.md) | Backlog Canvas: custom list component, Parall-Align-native data model (item/state model superseded by 0018) |
...
| [0009](docs/adr/0009-issue-persistence-and-versioning.md) | One JSON document per Issue; every view + the Backlog entry carries a stable UUID; git commit history is the version log (client-side for now); per-view history derived by field-level diffing (Issue-schema portion superseded by 0018) |
| [0010](docs/adr/0010-cross-issue-copy.md) | Copy a view/Backlog entry from another Issue: always HEAD, full overwrite, `copiedFrom` provenance (keyed on the source view's UUID) recorded, never a live link (superseded by 0019) |
...
| `src/shell/backlog-panel/` | Right panel: the Issue's `backlogEntries` list (add/rename/describe/delete), docked, global minimize-with-reset-on-activation. Issue `name`/`status` live in the view-switcher tab bar instead (ADR-0018) | 0005, 0007, 0008, 0018 |
...
| `src/persistence/` | Issue JSON model (ADR-0018 shape) + client-side git (`isomorphic-git`/`lightning-fs`/IndexedDB), debounced autosave-as-commit, per-view history browsing/restore, cross-issue copy | 0009, 0018 |
| `src/copy/` | Cross-issue copy mechanics: view copy by overwrite, Backlog-entry copy by list-append, never a whole Issue | 0019 |
...
- **Settings gear button for overall app settings.** No scope defined yet beyond: a gear icon somewhere in the shell chrome that opens an overlay. What settings live in it is still open, but one concrete candidate surfaced: an optional URL field on Backlog entries for external resources (Jira tickets or similar) — whether that's a per-entry field or an app-wide setting enabling/labeling it is still open too. May be worth looking at how `bpmn-process-creator`'s own UI handles an overlay/panel for inspiration on the mechanics (not a decision to cite it as architectural precedent the way ADR-0007 originally did before that got scrubbed — just an implementation reference for "how does an overlay open/close cleanly"). Direction given later: this belongs in the same app-wide control cluster as the theme toggle and a general-info overlay (see the two bullets below) — not a panel-scoped control.
- **General-info overlay.** New, unscoped — named only as a sibling to Settings and the theme toggle: something like an About/version/help overlay. No content, trigger, or placement decided yet; grouped here only because direction was "settings and general info overlay [are], like light/darkmode, a real 'overall' thing" — i.e. it belongs in the same app-wide cluster, not inside a panel.
- **Theme toggle: icon instead of text, relocated out of the Backlog panel.** The toggle (`.theme-toggle` in `.backlog-controls`, `index.html`) currently reads "Light mode"/"Dark mode" as plain text next to the Backlog minimize handle. Wanted: a lightbulb or sun/moon icon instead of text. It should also move out of the Backlog panel entirely — light/dark mode is an app-wide ("overall") concern like Settings and the general-info overlay above, not something that belongs docked to one panel's controls. Exact destination undecided; likely ends up clustered with the Settings gear and general-info trigger somewhere in shared shell chrome once that cluster exists.
- **Side-panel resize handles need a visible affordance, and dragging one to zero-width should minimize that panel.** Both the Issue sidebar and the Backlog panel already support drag-to-resize (`.resize-handle-right` / `.resize-handle-left` in `index.html`, `startResize()` in `shell-state.js`), but the hit-area is invisible — nothing currently shows it's there or draggable. Wanted: a visible handle indicator (grip marks, a highlighted strip, hover state) on both. Additionally, dragging a handle all the way to width 0 should minimize the panel to fully invisible, with the handle itself staying visible and draggable at that collapsed width so dragging it back out re-expands the panel. If this works well, the Backlog panel's existing explicit minimize toggle (`.backlog-toggle`, the ›/‹ button currently next to the theme toggle) becomes redundant and can be deleted — one interaction instead of two. Open question for whoever picks this up: the Issue sidebar currently collapses via a `<details>` element (`.sidebar-collapse`), a different mechanism entirely from the Backlog panel's toggle button — decide whether to unify both panels on the same drag-handle-minimize behavior, or leave the sidebar's `<details>` collapse as-is alongside the new handle affordance.
...
- No live cross-issue references (ADR-0019) — copy is a one-time snapshot (overwrite for views, append for Backlog entries), never a link that stays in sync with its source; a whole Issue is never copyable at all.
