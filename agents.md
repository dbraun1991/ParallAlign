...
**Current status: real persistence + dark-mode canvases + working All view + New-Issue creation + history/restore — data model mid-revision.** Product framing (`README.md`) and architecture decisions (`docs/adr/`) are in place; the Vite pipeline (ADR-0012), the Issue-shell (`src/shell/`, ADR-0007/0008/0013), the light/dark toggle (ADR-0014) — including System/Interaction (draw.io) and Object (Mermaid) per ADR-0015, but Process (bpmn-js) deliberately stays light-only per ADR-0016 — real git-backed persistence (`src/persistence/`, ADR-0009 — `isomorphic-git`/`lightning-fs` over IndexedDB, one JSON file per Issue, every save a commit), the All view (`src/canvases/thumbnails.js`, ADR-0017 — real per-engine SVG thumbnails, click-through, content-hash cached, not yet live-updating), New-Issue creation (the "+ New Issue" button in the sidebar, `src/persistence/issue-store.js`'s `createIssue()`), and per-view history browsing/restore (ADR-0009's other half, `getViewHistory`/`restoreView`, a "History" button on every single-canvas view) are all working end to end. Three example Issues still seed automatically on a true first run (`src/persistence/seed-issues.js`), now alongside any Issues created since.

**⚠ ADR-0018/ADR-0019 just revised the data model** — Issue narrows to `{id, name, status}`, Backlog becomes a `backlogEntries` list, cross-issue copy narrows to views + entries (never a whole Issue) — **but none of that is implemented yet.** The Backlog field editing UI currently in the code (title/theme/state/notes on the Issue itself, ADR-0005's original shape) is now the *old* model and needs rebuilding to match ADR-0018; `seed-issues.js` and any locally-persisted Issues are old-shape and will need clearing (`schemaVersion` bumped 1→2, no migration path — see ADR-0018). Treat ADR-0018/0019 as the plan, not as something already built.
...
| `src/shell/backlog-panel/` | Right panel: the Issue's `backlogEntries` list (add/rename/describe/delete), docked, global minimize-with-reset-on-activation. Issue `name`/`status` no longer live here — moved to the view-switcher tab bar (ADR-0018, not yet built) | 0005, 0007, 0008, 0018 |
...
- **Implement ADR-0018 + ADR-0019 (highest priority — current code is now stale).** The Backlog editing UI and Issue schema built earlier this session match the *old* model (title/theme/state/notes on the Issue). Needs: rewriting `seed-issues.js` to the new `{id, name, status, views, backlogEntries}` shape (`schemaVersion: 2`); clearing existing local IndexedDB state (`indexedDB.deleteDatabase('parall-align')`) since there's no migration path; moving the Issue `name`/`status` control from the Backlog panel into the view-switcher tab bar; rebuilding the Backlog panel as an entries list (add/rename/describe/delete, no status field); and the revised copy picker (view copy stays overwrite, entry copy is list-append, no whole-Issue copy option at all).
- **Settings gear button for overall app settings.** No scope defined yet beyond: a gear icon somewhere in the shell chrome that opens an overlay. What settings live in it is still open, but one concrete candidate surfaced: an optional URL field on Backlog entries for external resources (Jira tickets or similar) — whether that's a per-entry field or an app-wide setting enabling/labeling it is still open too. May be worth looking at how `bpmn-process-creator`'s own UI handles an overlay/panel for inspiration on the mechanics (not a decision to cite it as architectural precedent the way ADR-0007 originally did before that got scrubbed — just an implementation reference for "how does an overlay open/close cleanly").
...
- **Cross-issue copy picker UI** (ADR-0019) — mechanics decided for both view copy (overwrite) and Backlog-entry copy (list-append); choosing *which* Issue and which view/entry to copy from needs its own picker UI, not designed yet. (Superseded scope from the old ADR-0010-based TODO — folded into the ADR-0018/0019 implementation item above.)
...
- No live cross-issue references (ADR-0019) — copy is a one-time snapshot (overwrite for views, append for Backlog entries), never a link that stays in sync with its source; a whole Issue is never copyable at all; the copy feature itself isn't built yet either.
- No implementation yet of ADR-0018's revised Issue/Backlog-entries model — the Backlog UI currently in the code is the superseded shape (see the status note at the top of this file).
