...
- **Move the theme toggle button.** Currently `position: fixed` top-right of the whole viewport (`.theme-toggle` in `src/css/shell.css`, ADR-0014). Move it to sit directly above the Backlog panel's resize handle/maximize control instead — anchored to that panel rather than floating independently.
